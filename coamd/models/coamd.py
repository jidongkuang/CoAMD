...
from coamd.models.diffmlps import DiffMLPs_models
from coamd.evaluation.metrics import eval_decorator
from coamd.utils.train_utils import lengths_to_mask, uniform, get_mask_subset_prob, cosine_schedule
...
class MARDM(nn.Module):
    ...
    def forward_loss(self, latents, y, m_lens):
        ...
        input = latents.clone() ## （256, 196//4, 512）
        ...
        ## 为每个样本随机选择一定数量的 token 进行遮蔽,得到mask
        ...
        z = self.forward(input, cond_vector, ~non_pad_mask, force_mask)  ## （256, 196//4, 1024）
    ...
    def generate(self,
        ...
        for timestep, steps_until_x0 in zip(torch.linspace(0, 1, timesteps, device=device), reversed(range(timesteps))):
            rand_mask_prob = cosine_schedule(timestep)  ## timestep从小到大，被掩码的位置从大到小
            ...
            latents = torch.where(is_mask.unsqueeze(-1), logits, latents)  ## 只修改is_mask位置的latents

            # --- 开始奖励引导部分 ---
            if reward_model is not None and guidance_lambda > 0:
                with torch.enable_grad():
                    # 1. 准备输入
                    
                    latents_grad = latents.clone().requires_grad_(True)
                    latents_grad = torch.where(padding_mask.unsqueeze(-1), torch.zeros_like(latents_grad), latents_grad)
                    
                    # 2. 从潜变量解码出当前步骤的动作 (x_t -> x_0_hat)
                    # 注意：MARDM 直接操作 latents, AE 在外部。这里需要 AE.
                    current_motions = ae.decode(latents_grad.permute(0, 2, 1))
                    
                    # 3. 计算奖励 (文本-动作对齐分数)
                    # [您的优点1] 使用正确的离散时间步
                    # current_t = torch.tensor([steps_until_x0] * b, device=device)
                    fused_feat, joint_feat, bone_feat, motion_feat = reward_model.encode_3stream(current_motions, m_lens)
                    text_features = reward_model.encode_text(conds) # conds 是文本
                    
                    # 使用余弦相似度作为奖励
                    # 使用余弦相似度作为奖励
                    reward_fused = F.cosine_similarity(fused_feat, text_features, dim=-1)
                    reward_joint = F.cosine_similarity(joint_feat, text_features, dim=-1)
                    reward_bone = F.cosine_similarity(bone_feat, text_features, dim=-1)
                    reward_motion = F.cosine_similarity(motion_feat, text_features, dim=-1)

                    total_reward = (0.7 * reward_fused + 
                        0.1 * reward_joint + 
                        0.1 * reward_bone + 
                        0.1 * reward_motion).sum() # 使用 .sum() 来得到一个标量，以便于求导
                    
                    # 4. 计算奖励关于 latents_grad 的梯度
                    grad = torch.autograd.grad(total_reward, latents_grad)[0]
                    
                    # --- 5. [新增] 梯度归一化与尺度调整 ---
                    # 计算每个样本梯度的范数
                    # grad.flatten(1) 将 [B, L, D] -> [B, L*D]
                    grad_norm = torch.linalg.vector_norm(grad.flatten(1), dim=1, keepdim=True)
                    # 避免除以零
                    grad_normalized = grad / (grad_norm.view(-1, 1, 1) + 1e-8)

                    
                    # 5. 使用梯度更新 latents
                    # latents.shape: (B, L, D), grad.shape: (B, L, D)
                    latents = latents + (0.5+guidance_lambda) * grad_normalized * is_mask.unsqueeze(-1).float()
...
def mardm_sit_xl(**kwargs):
    return MARDM(latent_dim=1024, ff_size=4096, num_heads=16, dropout=0.2, clip_dim=512,
...
# Public aliases for the cleaned repository naming.
CoAMD = MARDM

def coamd_ddpm_xl(**kwargs):
    return mardm_ddpm_xl(**kwargs)

def coamd_sit_xl(**kwargs):
    return mardm_sit_xl(**kwargs)

COAMD_models = {
    'CoAMD-DDPM-XL': coamd_ddpm_xl,
    'CoAMD-SiT-XL': coamd_sit_xl,
    **MARDM_models,
}
